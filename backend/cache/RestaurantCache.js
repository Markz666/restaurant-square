...
exports.storeRestaurants = async function(data){
	let successCount = 0;
...
addRestaurant = async function (id, info, resolve, reject){
	await redisCache.setElement(id, info).then(resolve).catch(reject);
}

exports.getRestaurant = async function (id, resolve, reject){
 	await redisCache.getElement(id).then(resolve).catch(reject);
}

exports.removeRestaurant = async function (id, resolve, reject){
